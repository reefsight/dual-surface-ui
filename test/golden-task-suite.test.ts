...
const domain = "dual-surface-ui:agent-golden-task-suite:0.5\0";
...
const suitePath = resolve(process.cwd(), "fixtures", "evaluation", "golden-tasks-0.5.json");
const schemaPath = resolve(process.cwd(), "fixtures", "evaluation", "golden-task-suite-0.5.schema.json");
...
describe("P3.8 frozen golden-task suite", () => {
  it("covers the accepted task categories and binds a stable digest", async () => {
    ...
    expect(suite.schemaVersion).toBe("0.5");
    expect(suite.kind).toBe("agent-golden-task-suite");
    expect(suite.suiteId).toBe("phase3-golden-tasks");
    expect(suite.supportedDrivers).toEqual(["openai-gpt-5.6-luna-medium@0.5", "openai-gpt-5.6-luna-high@0.5"]);
