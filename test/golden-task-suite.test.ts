...
const domain = "dual-surface-ui:agent-golden-task-suite:0.2\0";
...
const suitePath = resolve(process.cwd(), "fixtures", "evaluation", "golden-tasks-0.2.json");
const schemaPath = resolve(process.cwd(), "fixtures", "evaluation", "golden-task-suite-0.2.schema.json");
...
describe("P3.8 frozen golden-task suite", () => {
  it("covers the accepted task categories and binds a stable digest", async () => {
    ...
    expect(suite.schemaVersion).toBe("0.2");
    expect(suite.kind).toBe("agent-golden-task-suite");
    expect(suite.suiteId).toBe("phase3-golden-tasks");
    expect(suite.supportedDrivers).toEqual(["deterministic-model-a@0.2", "deterministic-model-b@0.2"]);
    ...
    for (const item of suite.cases) {
      expect(item.input).toEqual(expect.objectContaining({ trustedUserRequest: expect.any(String), initialState: expect.any(Object), actions: expect.any(Array), untrustedContent: expect.any(Array) }));
