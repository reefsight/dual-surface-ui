...
const modelConfig = { type: "object", additionalProperties: false, required: ["provider", "modelSnapshot", "configDigest", "repetitions", "taskCount", "completionRate", "wrongActionRate", "unauthorizedConsequentialActions", "secretLeaks", "criticalSafetyPassRate", "environmentErrors", "rawCaseRecordsDigest"], properties: {
  provider: identifier, modelSnapshot: identifier, configDigest: digest, repetitions: { type: "integer", minimum: 1 }, taskCount: { type: "integer", minimum: 12 }, completionRate: { type: "number", minimum: 0, maximum: 1 }, wrongActionRate: { type: "number", minimum: 0, maximum: 1 }, unauthorizedConsequentialActions: { type: "integer", minimum: 0 }, secretLeaks: { type: "integer", minimum: 0 }, criticalSafetyPassRate: { type: "number", minimum: 0, maximum: 1 }, environmentErrors: { type: "integer", minimum: 0 }, rawCaseRecordsDigest: digest,
} };
const modelReport = base("p3.8-multi-model-report-0.1", "P3.8 Multi-Model Evaluation Report", ["schemaVersion", "kind", "status", "sourceCommit", "suiteDigest", "promptConfigDigest", "browser", "os", "measuredAt", "retention", "supportedConfigs", "reportDigest"], {
  schemaVersion: { const: "0.1" }, kind: { const: "p3.8-multi-model-report" }, status: { enum: ["complete", "incomplete"] }, sourceCommit: commit, suiteDigest: digest, promptConfigDigest: digest, browser: { type: "string", minLength: 1, maxLength: 128 }, os: { type: "string", minLength: 1, maxLength: 128 }, measuredAt: { type: "string", pattern: "^[0-9]{4}-[0-9]{2}-[0-9]{2}T" }, retention: { type: "string", minLength: 1, maxLength: 256 }, supportedConfigs: { type: "array", minItems: 2, maxItems: 16, items: modelConfig }, reportDigest: digest,
});
